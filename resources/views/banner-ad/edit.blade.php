    ...
    <section class="section banner-wizard">
        <div class="card">
            <div class="card-body">
                ...
                <form id="create-form" method="POST" action="{{ route('banner-ad.update', $editData['id']) }}" enctype="multipart/form-data" data-success-function="successFunction">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="p-3">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-lg-6 d-none banner-field-category">
                                            ...
                                            <div class="banner-category-warning d-none mt-1" style="color:#b8860b;font-size:.85rem;"></div>
                                        </div>
                                        <div class="col-lg-6 d-none banner-field-advertisement">
                                            ...
                                            <div class="banner-advertisement-warning d-none mt-1" style="color:#b8860b;font-size:.85rem;"></div>
                                        </div>
...
@section('script')
    <script>
        (function () {
            ...
            function wireAccordion(node, index) {
                ...
                node.querySelector('.dz-remove').addEventListener('click', (e) => {
                    ...
                    bannerState[index - 1].hasExisting = false;
                    previewImg.src = '';
            ...
            function validateStep3() {
                const filesOk = bannerState.length > 0 && bannerState.every(b => !!b.file || b.hasExisting);
            ...
            function fetchExistingBanners() {
                ...
                fetch(EXISTING_BANNERS_URL + '?platform=' + encodeURIComponent(selections.platform) +
                    ...
                    .then(r => r.json())
                    .then(j => {
                        // Exclude the unit currently being edited — it is the "new" banner here.
                        existingBanners = (j.data || []).filter(rec => {
                            if (EDIT.group_id) return rec.group_id !== EDIT.group_id;
                            return !(EDIT.banner_ids || []).includes(rec.id);
                        });
            ...
            function renderAdsDetailPreview() {
                ...
                const previews = bannerState.map(b => b.file ? URL.createObjectURL(b.file) : (b.existingImage || null));
                const spec = bannerSizeSpec();
                const aspectStyle = spec ? `style="--bnr-ratio:${(spec.h / spec.w * 100).toFixed(3)}%;"` : '';
            ...
            // ===== Edit prefill =====
            const EDIT = @json($editData);

            function prefillCard(group, value) {
                const el = document.querySelector(
                    `.option-card[data-group="${group}"][data-value="${value}"],`
                    + `.layout-card[data-group="${group}"][data-value="${value}"]`
                );
                if (el) el.click();
            }

            const CATEGORY_INDEX_URL = "{{ route('category.index') }}";
            const ADVERTISEMENT_INDEX_URL = "{{ route('advertisement.index') }}";

            function stateWarningHtml(state, listUrl) {
                if (!state || state.status === 'ok') return '';
                if (state.status === 'deleted') {
                    return `<i class="ph-fill ph-warning-circle"></i> {{ __('This item was deleted. Please select a new one.') }}`;
                }
                const link = `<a href="${listUrl}?search=${encodeURIComponent(state.name || '')}" target="_blank" rel="noopener">{{ __('Enable it') }}</a>`;
                return `<i class="ph-fill ph-warning-circle"></i> {{ __('Currently disabled — still selected, but hidden from new selections.') }} ${link}`;
            }

            function showCategoryWarning(node, state) {
                const el = node.querySelector('.banner-category-warning');
                if (!el) return;
                const html = stateWarningHtml(state, CATEGORY_INDEX_URL);
                el.innerHTML = html;
                el.classList.toggle('d-none', !html);
            }

            function showAdvertisementWarning(node, state) {
                const el = node.querySelector('.banner-advertisement-warning');
                if (!el) return;
                const html = stateWarningHtml(state, ADVERTISEMENT_INDEX_URL);
                el.innerHTML = html;
                el.classList.toggle('d-none', !html);
            }

            function prefillBanners() {
                const nodes = document.querySelectorAll('#banner-accordions .banner-accordion');
                nodes.forEach((node, idx) => {
                    const data = EDIT.banners[idx];
                    if (!data) return;
                    const state = bannerState[idx];

                    if (data.image) {
                        node.querySelector('.dz-preview-img').src = data.image;
                        node.querySelector('.dz-preview-wrap').classList.remove('d-none');
                        node.querySelector('.dz-placeholder').classList.add('d-none');
                        state.hasExisting = true;
                        state.existingImage = data.image;
                    }

                    const typeSelect = node.querySelector('.banner-ad-type');
                    typeSelect.value = data.ad_type;
                    typeSelect.dispatchEvent(new Event('change'));

                    if (data.ad_type === 'category') {
                        const catSelect = node.querySelector('.banner-category');
                        catSelect.value = data.category_id != null ? String(data.category_id) : '';
                        if (window.jQuery && jQuery.fn.select2) {
                            jQuery(catSelect).trigger('change');
                        } else {
                            catSelect.dispatchEvent(new Event('change'));
                        }
                        showCategoryWarning(node, data.category_state);
                    } else if (data.ad_type === 'advertisement') {
                        const catSelect = node.querySelector('.banner-category');
                        const adSelect = node.querySelector('.banner-advertisement');
                        catSelect.value = data.category_id != null ? String(data.category_id) : '';
                        if (window.jQuery && jQuery.fn.select2) {
                            jQuery(catSelect).trigger('change.select2');
                        }
                        state.category = catSelect.value;
                        showCategoryWarning(node, data.category_state);
                        showAdvertisementWarning(node, data.advertisement_state);
                        if (data.category_id) {
                            adSelect.innerHTML = '<option value="">{{ __('Loading...') }}</option>';
                            adSelect.disabled = true;
                            fetch(ITEMS_BY_CAT_URL + '?category_id=' + encodeURIComponent(data.category_id), {
                                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' }
                                })
                                .then(r => r.json())
                                .then(j => {
                                    ...
                                    adSelect.innerHTML = '<option value="">{{ __('Select Advertisement') }}</option>' +
                                        items.map(it => `<option value="${it.id}">${it.name}</option>`).join('');
                                    adSelect.disabled = items.length === 0;
                                    if (data.advertisement_id != null) {
                                        // Fetched list only has active/approved items — inject the
                                        // saved advertisement if it's disabled/expired so it still shows selected.
                                        const st = data.advertisement_state;
                                        const already = items.some(it => String(it.id) === String(data.advertisement_id));
                                        if (!already && st && st.status !== 'deleted' && st.name) {
                                            const opt = document.createElement('option');
                                            opt.value = String(data.advertisement_id);
                                            opt.textContent = st.name + ' ({{ __('Disabled') }})';
                                            adSelect.appendChild(opt);
                                            adSelect.disabled = false;
                                        }
                                        adSelect.value = String(data.advertisement_id);
                                        state.ad = adSelect.value;
                                    }
                                    validateStep3();
                                })
                    ...
                    } else if (data.ad_type === 'external_link') {
                        const linkInput = node.querySelector('.banner-external-link');
            ...
            // Move the editable banner section to the spot the saved record points at.
            function positionBannerForEdit() {
                if (EDIT.layout === 'single_side' || EDIT.layout === 'dual_side') return;
                const bannerEntry = placementOrder.find(s => s.id === 'banner_new');
                if (!bannerEntry) return;
                const order = placementOrder.filter(s => s.id !== 'banner_new');

                let anchorIdx = -1;
                if (EDIT.page === 'home') {
                    if (!EDIT.feature_section_id && !EDIT.home_screen_section_id) {
                        anchorIdx = order.findIndex(s => s.id === 'all_advertisement');
                    } else {
                        anchorIdx = order.findIndex(s => {
                            if (EDIT.feature_section_id) {
                                return String(s.feature_id ?? '') === String(EDIT.feature_section_id);
                            }
                            return s.hs_id != null
                                && String(s.hs_id) === String(EDIT.home_screen_section_id)
                                && !s.feature_id;
                        });
                    }
                } else if (EDIT.page === 'detail') {
                    anchorIdx = order.findIndex(s => DETAIL_SECTION_ENUM[s.id] === EDIT.detail_page_section);
                } else if (EDIT.page === 'listing') {
                    anchorIdx = order.findIndex(s => LISTING_SECTION_ENUM[s.id] === EDIT.listing_page_section);
                }

                if (anchorIdx === -1) {
                    placementOrder = [bannerEntry, ...order];
                } else {
                    const insertAt = EDIT.placement === 'above' ? anchorIdx : anchorIdx + 1;
                    order.splice(insertAt, 0, bannerEntry);
            ...
            function prefillEdit() {
                prefillCard('platform', EDIT.platform);
                applyPagesVisibility();
                prefillCard('page', EDIT.page);
                applyLayoutVisibility();
                prefillCard('layout', EDIT.layout);

                buildBannerAccordions();
                prefillBanners();
                fetchExistingBanners();
                buildPlacement();
                positionBannerForEdit();
        ...
        function successFunction(response) {
            setTimeout(() => {
                window.location.href = "{{ route('banner-ad.index') }}";
            }, 500);
        }
    </script>
@endsection
