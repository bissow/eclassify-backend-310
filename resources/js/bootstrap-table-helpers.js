...
window.BootstrapTableFormatters = {
    ...
    actionFormatter: function(value, row, index) {
        let actions = '';
        const icons = window.ActionIcons || {};

        if (window.canEdit !== false) {
            actions += '<a href="' + (row.edit_url || '#') + '" class="action-icon action-icon-edit me-1" title="' + window.trans('Edit') + '">' + (icons.edit || '') + '</a>';
        }

        if (window.canDelete !== false) {
            actions += '<button type="button" class="action-icon action-icon-delete me-1" onclick="deleteItem(' + row.id + ')" title="' + window.trans('Delete') + '">' + (icons.delete || '') + '</button>';
        }

        if (row.view_url) {
            actions += '<a href="' + row.view_url + '" class="action-icon action-icon-view me-1" title="' + window.trans('View') + '">' + (icons.view || '') + '</a>';
        }
