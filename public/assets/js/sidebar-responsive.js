...
(function () {
    ...
    function initSidebar() {
        ...
        document.addEventListener('click', function (e) {
            ...
            // Click outside the sidebar collapses it (mobile/tablet only).
            // Ignore clicks on the burger (it toggles) and inside the sidebar.
            if (window.innerWidth < 1200 && sidebar.classList.contains('active')) {
                if (!e.target.closest('#sidebar') && !e.target.closest('.burger-toggle')) {
                    closeSidebar();
                }
            }
        });
        ...
        window.addEventListener('resize', function () {
            ...
            resizeTimeout = setTimeout(function () {
                ...
                // 1200 matches the CSS breakpoint where #main regains its
                // 300px left margin; below it the fixed sidebar overlays
                // content, so it must stay closed until toggled.
        ...
        // Initialize sidebar state based on screen size. Use 1200 to match
        // the CSS breakpoint (#main loses its left margin below 1200, so the
        // fixed sidebar overlays content there).
        function initializeSidebar() {
            if (window.innerWidth >= 1200) {
                // Desktop: sidebar open by default
                sidebar.classList.add('active');
            } else {
                // Mobile/tablet: sidebar closed by default, opens on burger tap
                sidebar.classList.remove('active');
            }
        }
