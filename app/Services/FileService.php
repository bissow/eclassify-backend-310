...
class FileService
{
    ...
    public static function compressAndUpload($requestFile, string $folder, bool $addWaterMark = false)
    {
        ...
        $fileName = Str::slug($filenameWithoutExt) . '-' . time() . '.' . $extension;
        ...
        try {
            if (in_array($extension, ['jpg', 'jpeg', 'png', 'webp'])) {
                ...
                $image = Image::make($requestFile)->orientate()->encode($extension, 80);
    ...
    {
        try {
            return Storage::disk(config('filesystems.default'))->exists($filePath);
        } catch (Exception $e) {
