...
class PayPalPayment implements PaymentInterface
{
    ...
    /**
     * Fetch a capture's details from PayPal, used to authoritatively verify a webhook
     * event server-side (the panel stores no PayPal Webhook ID for signature checks).
     * Returns the decoded capture resource, or null on failure.
     */
    public function getCapture(string $captureId): ?array
    {
        try {
            $accessToken = $this->generateAccessToken();
            $response = $this->http->get("/v2/payments/captures/{$captureId}", [
                'headers' => [
                    'Authorization' => "Bearer {$accessToken}",
                    'Content-Type'  => 'application/json'
                ]
            ...
            Log::channel('webhook')->error('PayPal getCapture failed: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function verifyWebhookSignature(array $headers, string $payload, string $webhookId): bool
    {
        try {
            ...
            if (!$verification->successful()) {
                Log::channel('webhook')->error('PayPal verifyWebhookSignature failed: ' . $verification->body());
                return false;
            }

            return ($verification->json()['verification_status'] ?? '') === 'SUCCESS';
        } catch (Throwable $e) {
            Log::channel('webhook')->error('PayPal verifyWebhookSignature exception: ' . $e->getMessage());
            return false;
        }
    }
}
