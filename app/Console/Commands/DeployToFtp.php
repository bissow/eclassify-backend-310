...
class DeployToFtp extends Command
{
    protected $signature = 'deploy:ftp
        {commit? : Commit hash to diff against its parent, or a range A..B (default: HEAD)}
        {--profile=dev : Profile key from .vscode/sftp.json (dev|stage)}
    ...
    public function handle(): int
    {
        ...
        if (! $profile) {
            $this->error("Profile '{$profileKey}' not found in sftp.json");
            return self::FAILURE;
        }
        ...
        $profileName = $profile['name'] ?? $profileKey;

        $this->info('Profile: ' . $profileName . ' (' . $profile['host'] . ')');
        ...
        if (! $this->confirm('Upload these files to ' . $profileName . '?', true)) {
            $this->warn('Aborted.');
            return self::SUCCESS;
        }
    ...
    private function changedFiles(string $commit): array
    {
        if (str_contains($commit, '..')) {
            [$from, $to] = array_pad(explode('..', $commit, 2), 2, '');
            $to = $to !== '' ? $to : 'HEAD';
            $command = ['git', 'diff', '--name-only', $from, $to];
        } else {
