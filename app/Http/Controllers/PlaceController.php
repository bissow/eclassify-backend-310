...
class PlaceController extends Controller
{
    ...
    public function countryShow(Request $request)
    {
        try {
            ...
            $sql = Country::select(['id', 'name', 'emoji', 'status']);
    ...
    public function stateSearch(Request $request)
    {
        try {
            ...
            $states = State::where(['country_id' => $request->country_id, 'status' => true])->select(['id', 'name'])->orderBy('name', 'ASC')->get();
        ...
        try {
            ...
            foreach ($result as $key => $row) {
                ...
                if (auth()->user()->can('state-delete')) {
                    $tempRow['operate'] = BootstrapTableService::deleteButton(route('states.destroy', $row->id));
                }
                $rows[] = $tempRow;
            }
    ...
    public function destroyState(string $id)
    {
        try {
            ResponseService::noPermissionThenSendJson('state-delete');
            State::findOrFail($id)->delete();
            ResponseService::successResponse('State deleted successfully');
        } catch (Throwable $th) {
            ResponseService::logErrorResponse($th, 'Place Controller -> destroyState');
            ResponseService::errorResponse('Something Went Wrong');
        }
    }
    ...
    public function citySearch(Request $request)
    {
        try {
            ...
            $cities = City::where(['state_id' => $request->state_id, 'status' => true])->select(['id', 'name'])->orderBy('name', 'ASC')->get();
    ...
    public function createArea()
    {
        ...
        return view('places.area', compact('countries', 'states'));
    }
