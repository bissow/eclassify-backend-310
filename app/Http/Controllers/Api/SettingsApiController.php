...
class SettingsApiController extends BaseApiController
{
    ...
    public function getSystemSettings(Request $request)
    {
        try {
            ...
            if (! empty($request->type)) {
                ...
                $query->whereIn('name', ['demo_mode','android_version','ios_version','default_language','current_language','force_update','android_maintenance_mode','ios_maintenance_mode','web_maintenance_mode','free_ad_listing','otp_service_provider','test_otp_code','map_provider','google_map_key','currency_symbol','currency_iso_code','currency_symbol_position','banner_ad_status','banner_ad_id_android','banner_ad_id_ios','interstitial_ad_status','interstitial_ad_id_android','interstitial_ad_id_ios','native_ad_status','native_app_id_android','native_app_id_ios','play_store_link','app_store_link','default_latitude','default_longitude','min_length','max_length', 'email_authentication','mobile_authentication','google_authentication','apple_authentication','languages','about_us','adsense_banner_slot_id','adsense_client_id','adsense_enabled','adsense_mode','adsense_square_slot_id','adsense_vertical_slot_id','company_address','company_email','company_logo','company_name','company_tel1','company_tel2','contact_us','deep_link_scheme','facebook_link','favicon_icon','footer_description','footer_logo','google_map_iframe_link','header_logo','instagram_link','linkedin_link','pinterest_link','placeholder_image','privacy_policy','refund_policy','show_landing_page','terms_conditions','x_link', 'web_theme_color', 'web_setup', 'web_url', 'refer_earn_enabled','refer_points_for_referrer','refer_points_for_referred','refer_max_points_usage_percentage','refer_min_points_to_use','refer_max_points_to_use','gemini_ai_enabled', 'feature_item_sorting', 'general_item_sorting', 'featured_items_weightage','max_gallery_images','reel_max_file_size_mb','reel_max_duration_sec','item_video_max_file_size_mb']);
            }
            ...
            $tempRow['languages'] = CachingService::getLanguages()->where('status', true)->values();
    ...
    public function getCurrencies(Request $request)
    {
        try {
            ...
            $currencies = Currency::select('id', 'iso_code', 'symbol', 'symbol_position', 'country_id', 'decimal_places', 'thousand_separator', 'decimal_separator')
                ->orderBy('iso_code')
                ->get()
    ...
    public function getLanguages(Request $request)
    {
        try {
            ...
            $language = Language::where(['code' => $request->language_code, 'status' => true])->first();
            if(collect($language)->isEmpty()){
                ResponseService::errorResponse(__('Language not found'), []);
            }
    ...
    public function getSystemLanguagesCodes()
    {
        try {
            ...
            $otherLanguageData = $languagesQuery->clone()->where('id','!=',$defaultLanguageData->id)->where('status', true)->select('id', 'code')->get();
            $languageData = array(
