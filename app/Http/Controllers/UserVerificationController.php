    ...
    public function show(Request $request)
    {
        try {
            ...
            foreach ($result as $row) {
                $row->verification_fields = collect($row->verification_fields)->map(function ($verification_field) use ($verificationFieldValues, $row, $defaultLanguageId) {
                    // Default value = value stored under default language (seller submits form data with default language only)
                    $fieldValue = $verificationFieldValues->first(function ($data) use ($row, $verification_field, $defaultLanguageId) {
                        return $data->verification_field_id == $verification_field->id
                            && $data->verification_request_id == $row->id
                            && ($data->language_id == $defaultLanguageId || empty($data->language_id));
                    });
                ...
                if (Auth::user()->can('seller-verification-request-update')) {
                    ...
                    $operate .= BootstrapTableService::button('ph ph-eye', '#', ['view-verification-fields'], ['title' => __('View'), 'data-bs-target' => '#editModal', 'data-bs-toggle' => 'modal']);
                }
                $tempRow = $row->toArray();
