...
class RoleController extends Controller {
    ...
    public function list(Request $request) {
        ...
        $no = 1;
        foreach ($res as $row) {
            $operate = BootstrapTableService::button('ph ph-eye', route('roles.show', $row->id), [], ['title' => 'View']);
    ...
    public function create() {
        ...
        $excludePermission = [
            ...
            'featured-advertisement-package-delete',
            'advertisement-package-create',
            'advertisement-package-list',
            'advertisement-package-update',
            'advertisement-package-delete'
        ];            

        foreach ($permission as $key => $val) {
            if (in_array($val->name, $excludePermission, true)) {
                continue;
            }
