...
class StaffController extends Controller {
    ...
    public function show(Request $request) {
        ...
        foreach ($result as $key => $row) {
            ...
            if (Auth::user()->can('staff-update')) {
                ...
                $operate .= BootstrapTableService::editButton(route('staff.change-password', $row->id), true, '#resetPasswordModel', null, $row->id, 'ph ph-key');
            }
            ...
            $tempRow = HelperService::maskDemoUserData($row->toArray());
    ...
    public function destroy($id) {
        try {
            ...
            $userDeleteQuery = User::withTrashed()->findOrFail($id);
            if($adminUserID){
                $userDeleteQuery->where('id', '!=', $adminUserID);
            }
            $userDeleteQuery->forceDelete();
    ...
    public function changePassword(Request $request, $id) {
        ...
        try {
            User::where('id', '!=', 1)->where('id', $id)->first()->update(['password' => Hash::make($request->confirm_password)]);
